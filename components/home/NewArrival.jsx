...
const NewArrival = ({ settings = {}, products = [] }) => {
  const ctaText  = settings.ctaText  || "Explore New Arrivals";
  const ctaLink  = settings.ctaLink  || "/listing";

  const arrivals = products.length > 0
    ? products.map((p, i) => ({
        id:    p._id || i,
        title: p.name,
        sub:   p.metal?.purity ? `${p.metal.purity} ${p.metal.type || "Gold"}` : "New",
  ...
  return (
    <>
      <style>{`
        ...
        .na-card img { width: 100%; height: 200px; object-fit: cover; display: block; filter: brightness(0.88); transition: filter 0.3s; background: #1A0D05; }
        ...
        .na-card-title { font-family: 'Cormorant Garamond', serif; font-size: 20px; font-weight: 400; color: #FFFDF9; margin-bottom: 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .na-card-price { font-family: 'Jost', sans-serif; font-size: 12px; color: #9E8875; }
        .na-cards-tag { font-family: 'Jost', sans-serif; font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: #D4AF6A; display: flex; align-items: center; gap: 10px; padding-top: 32px; }
      ...
      <section className="na-section">
        <div className="na-hero">
          <div className="na-hero-text">
            ...
            <Link href={ctaLink} className="na-hero-cta">{ctaText} →</Link>
          </div>
        </div>
        <div className="na-gold-line" />
        <div className="na-cards-wrap">
          <p className="na-cards-tag">Latest Drops</p>
          <div className="na-cards">
            {arrivals.map(item => (
              <Link href={item.slug ? `/product?slug=${item.slug}` : "/listing"} key={item.id} className="na-card">
